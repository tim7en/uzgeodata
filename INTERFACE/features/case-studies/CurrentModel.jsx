...
export default function CurrentModel() {
  ...
  return <section id="current-model" className="cs-panel">
    <span className="cs-eyebrow">EARLIER PUBLISHED REFERENCE / {data.model.timestep.toUpperCase()} TIMESTEP</span>
    ...
    <Chart rows={rows} fields={[{key:'observed',label:'Observed',color:'#58c9e5'},{key:'simulated',label:'Reference fit',color:'#edb06c'}]} title={`${view==='monthly'?'Monthly runoff':'Seasonal volume'} · reference ${split}`} unit={view==='monthly'?'mm/day':'million m³'}/>
    ...
    <div className="cs-science-warning"><strong>Historical skill, not an operational forecast</strong><p>{data.scope}</p><p>{data.catchment.caveat}</p><p>This block uses the stratified split, which places wet and dry years on both sides of the divide. The seasonal volume score above is not comparable with the chronological evaluation at the top of the page, where every test year follows every calibration year and the seasonal volume loses its skill.</p></div>
