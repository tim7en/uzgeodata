...
const datasets = [
  { title: 'Protected Areas', category: 'Biodiversity', type: 'LPKX', size: '0.4 MB', access: 'Free', icon: Leaf, desc: 'Sixty-two protected-area polygons with designation, location and recorded area attributes.' },
  { title: 'Earthquakes 1990–2024', category: 'Hazards', type: 'LPKX', size: '0.8 MB', access: 'Free', icon: Orbit, desc: 'Regional earthquake observations with magnitude, depth, location and event time.' },
  { title: 'Water Management Zones', category: 'Water', type: 'LPKX', size: '0.2 MB', access: 'Free', icon: Droplets, desc: 'Irrigated areas and water-management zones digitized for national-scale analysis.' },
  { title: 'Flood Risk', category: 'Hazards', type: 'LPKX', size: '0.4 MB', access: 'Free', icon: Layers3, desc: 'Catchment-level riverine flood risk scores and standardized risk categories.' },
  { title: 'National Land Cover', category: 'Raster', type: 'LPKX', size: '971 MB', access: 'Request', icon: Grid3X3, desc: 'The archive’s largest classified raster package, retained in its original ArcGIS format.' },
  { title: 'Maximum NDVI 2004–2024', category: 'Agriculture', type: 'LPKX', size: '132 MB', access: 'Request', icon: Mountain, desc: 'Long-term maximum vegetation-index surface for agricultural and ecosystem assessment.' },
];
...
const useCases = [
  ...
  { n:'06', sector:'Research', title:'Build reproducible evidence', icon:FlaskConical, question:'Can source layers be compared across themes, regions and time?', outcome:'Create cited analysis without rebuilding the data inventory.', data:['134 indexed layers','Source metadata','Ontology model'], users:'Universities · Research teams' },
];
...
function Hero({ onExplore, onAccess }) {
  return <section className="hero" id="top">
    ...
    <div className="hero-stats">
      <div><strong>134</strong><small>Atlas packages indexed</small></div>
      <div><strong>5</strong><small>Interactive map layers</small></div>
      <div><strong>1.6<span>GB</span></strong><small>Source archive</small></div>
    </div>
...
function Catalog({ onRequest }) {
  ...
  const categories = ['All data', 'Biodiversity', 'Hazards', 'Water', 'Raster', 'Agriculture'];
  ...
  return <section className="section catalog-section" id="catalog">
    <div className="section-head">
      ...
      <p>A focused selection from the environmental atlas. Source packages are indexed with English titles, access status and preserved original metadata.</p>
    </div>
...
function EnvironmentalMap() {
  ...
  const categories = ['All themes',...new Set(catalog.map(item=>item.category))];
  const filtered = catalog.filter(item => (category === 'All themes' || item.category === category) && `${item.title} ${item.sourceTitle}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <section className="map-explorer" id="map"><div className="map-intro"><div><div className="kicker">02 / Interactive atlas</div><h2>See the data.<br/><span>Read the terrain.</span></h2></div><p>Five source layers have been optimized for live exploration. Pan, zoom, switch themes and inspect individual features directly from the atlas archive.</p></div>
    <div className="map-workspace"><div className="map-stage"><MapContainer center={[41.2,64.6]} zoom={5} minZoom={4} maxZoom={12} zoomControl={false} preferCanvas>
      ...
      <aside className="map-panel"><div className="layer-heading"><span>LIVE LAYERS</span><small>{layers.length} / 134 web-ready</small></div><div className="layer-buttons">{layers.map(layer=><button key={layer.id} className={active===layer.id?'active':''} onClick={()=>setActive(layer.id)}><i style={{background:mapColors[layer.id]}}/><span><strong>{layer.title}</strong><small>{layer.features.toLocaleString()} features · {layer.geometry}</small></span><ArrowRight size={14}/></button>)}</div>
        <div className="archive-index"><div className="layer-heading"><span>FULL ARCHIVE INDEX</span><small>{filtered.length} packages</small></div><div className="archive-search"><Search size={15}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search all layers..."/></div><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(item=><option key={item}>{item}</option>)}</select><div className="archive-results">{filtered.slice(0,60).map(item=><div key={item.id}><span>{item.atlasNumber || '—'}</span><p><strong>{item.title}</strong><small>{item.category} · {(item.size/1024/1024).toFixed(item.size>10*1024*1024?0:1)} MB</small></p></div>)}</div>{filtered.length>60&&<div className="archive-more">+ {filtered.length-60} more matching packages</div>}</div>
      </aside></div>
  </section>
}
...
function OntologyExplorer({ onRequest }) {
  ...
  const graph = useMemo(()=>{
    ...
    ontologyDomains.forEach(domain=>{
      const items=catalog.filter(item=>item.category===domain.name);
      items.forEach((item,index)=>{
        ...
        datasetNodes.push({...item,x:domain.x+Math.cos(angle)*radius,y:domain.y+Math.sin(angle)*radius,domain,concept:inferConcept(item.title)});
      });
    });
    return datasetNodes;
  },[catalog]);
  ...
  const isVisible=node=>(activeDomain==='All domains'||node.category===activeDomain)&&(!queryLower||`${node.title} ${node.sourceTitle}`.toLowerCase().includes(queryLower));
  const matchCount=graph.filter(isVisible).length;
  const selectedConcept=selected&&ontologyConcepts[selected.concept];
  return <section className="ontology" id="ontology"><div className="ontology-intro"><div><div className="kicker">03 / Spatial ontology</div><h2>From files to<br/><span>knowledge.</span></h2></div><p>The atlas is modeled as a connected knowledge system. Explore how every dataset belongs to a domain, describes an analytical concept and contributes to the national environmental picture.</p></div>
    <div className="ontology-toolbar"><div className="ontology-search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Find an entity or concept..."/><span>{matchCount} objects</span></div><div className="ontology-filters"><button className={activeDomain==='All domains'?'active':''} onClick={()=>setActiveDomain('All domains')}>All domains</button>{ontologyDomains.map(domain=><button key={domain.name} className={activeDomain===domain.name?'active':''} onClick={()=>setActiveDomain(domain.name)}><i style={{background:domain.color}}/>{domain.name}</button>)}</div></div>
    <div className="ontology-workspace"><div className="ontology-canvas"><div className="ontology-controls"><button onClick={()=>setZoom(value=>Math.min(1.35,value+.1))} aria-label="Zoom in"><Plus/></button><button onClick={()=>setZoom(value=>Math.max(.72,value-.1))} aria-label="Zoom out"><Minus/></button><button onClick={()=>setZoom(1)} aria-label="Reset zoom"><RotateCcw/></button></div>
      <svg viewBox="0 0 1200 740" role="img" aria-label="Knowledge graph of Uzbekistan environmental datasets"><g style={{transform:`translate(600px, 370px) scale(${zoom}) translate(-600px, -370px)`,transformOrigin:'0 0'}}>
        ...
        <g className="root-node" transform="translate(600 360)"><circle r="49"/><circle r="38"/><Network size={25} x="-12.5" y="-23"/><text y="16">UZGEODATA</text><text className="root-subtitle" y="29">ATLAS ONTOLOGY</text></g>
...
function Community() {
  return <section className="community" id="community">
    ...
    <div className="community-copy">
      ...
      <div className="quote-person"><div className="avatar">UZ</div><div><strong>UzGeoData principle</strong><span>Source context stays connected to every layer</span></div></div>
      <div className="community-proof"><div><strong>134</strong><span>Environmental layers indexed</span></div><div><strong>7</strong><span>Connected knowledge domains</span></div></div>
...
function Standards() {
  return <section className="section standards" id="about">
    ...
    <div className="standard-list">
      {[['01','Source-preserving workflow','Original ArcGIS packages remain separate from lightweight browser derivatives.'],['02','Machine-readable index','English titles, domains, source filenames and sizes are available as structured metadata.'],['03','Semantic organization','Every layer connects to a domain and analytical concept through the ontology model.'],['04','Explicit access status','Free, request-based, licensed and internal data are distinguished before delivery.']].map(x => <div className="standard-item" key={x[0]}><span>{x[0]}</span><div><h3>{x[1]}</h3><p>{x[2]}</p></div><Check size={20}/></div>)}
    </div>
  </section>
}
...
function AccessModal({ initial, onClose }) {
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}><div className="modal">
    ...
    {!done ? <><div className="kicker">Data request</div><h2>Let’s find your<br/><span>best source.</span></h2><p>Describe the decision or analysis you are supporting. Your request will be recorded for access and licensing review.</p><form onSubmit={submit}>
      <label>YOUR NAME<input name="name" required placeholder="Full name"/></label><label>WORK EMAIL<input name="email" required type="email" placeholder="you@organization.uz"/></label><label>ORGANIZATION<input name="organization" placeholder="Agency, company or institution"/></label><label>DATASET OR TOPIC<input name="topic" required defaultValue={initial || ''} placeholder="e.g. flood risk, NDVI..."/></label><label>INTENDED USE<textarea name="intendedUse" rows="3" placeholder="Decision, study area, timeframe and preferred format"/></label>{error&&<div className="form-error">{error}</div>}<button className="button" type="submit" disabled={loading}>{loading?<><LoaderCircle className="spin" size={17}/> Recording request…</>:<>Submit data request <ArrowRight size={17}/></>}</button>
    </form></> : <div className="success"><span><Check size={32}/></span><h2>Request recorded.</h2><p>Thank you. Your project context and dataset needs have been saved for review.</p><button className="button" onClick={onClose}>Back to the portal</button></div>}
  </div></div>
}
