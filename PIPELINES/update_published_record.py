...
query cube. This extends those two directly:
  ...
  1. The last month each variable of the source holds is read from the cube.
...
def published_through(attributes, cube=CUBE):
    ...
    for attribute in attributes:
        ...
        if not path.exists():
            raise FileNotFoundError(f"{path.relative_to(ROOT) if path.is_relative_to(ROOT) else path} is missing")
...
def update_history(rows, attributes, first, through, note, history=HISTORY):
    ...
    index.setdefault("appended", []).append(note)
...
def update_cube(rows, attributes, first, note, cube=CUBE):
    ...
    for attribute in attributes:
        ...
        connection.execute(f"""
            COPY (
                SELECT basin_id, attribute_id, year, month, value, unit FROM (
    ...
    summary.setdefault("appended", []).append(note)
...
def update(source, through, dry_run=False, cube=CUBE, history=HISTORY):
    ...
    print(f"{source}: published through {stored}; fetching {first} to {through}", flush=True)
    if dry_run:
    ...
    rows = extracted_rows(store, attributes, first, through)
    check_complete(rows, attributes, ledger["basins"], first, through)
    ...
    basins = update_history(rows, attributes, first, through, note, history)
    cube_rows = update_cube(rows, attributes, first, note, cube)
    return {"ok": True, "source": source, "fetched": [first, through], "values": len(rows),
